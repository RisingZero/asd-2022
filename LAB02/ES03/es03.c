/* 
    LAB2 ES03 RAINERI ANDREA ANGELO S280848
...
enum comando_e { r_stampa, r_carica, r_ord_data, r_ord_codice, r_ord_partenza, r_ord_capolinea, r_ricerca, r_fine };
enum ord_type_e { data, codice, partenza, capolinea };

int readFile(corsa_t **corse, corsa_t ***corse_ord, int newRead);

enum comando_e leggiComando();
void eseguiComando(enum comando_e comando, corsa_t corse[], corsa_t **corse_ord[], int *numCorse);
...
int main(int argc, char const *argv[]) {
    ...
    int numCorse, i, j;
    corsa_t *corse;
    corsa_t **corse_ord[NUM_ORDS];
    numCorse = readFile(&corse, corse_ord, 0);

    do {
        comando = leggiComando();
        if (comando != r_fine)
            eseguiComando(comando, corse, corse_ord, &numCorse);
...
int readFile(corsa_t **corse, corsa_t ***corse_ord, int newRead) {
    ...
    if (newRead) {
        printf("Inserire nome nuovo file: ");
        scanf("%s", fname);
        fp = fopen(fname, "r");
    } else {
        fp = fopen(FILENAME, "r");
    ...
    *corse =  (corsa_t *) malloc(numCorse * sizeof(corsa_t));
    ...
    for (i = 0; i < NUM_ORDS; i++) {
        corse_ord[i] = (corsa_t**) malloc(numCorse * sizeof(corsa_t*));
        for (j = 0; j < numCorse; j++) {
            corse_ord[i][j] = &(*corse)[j];
        }
    }
...
// Command handler
void eseguiComando(enum comando_e comando, corsa_t corse[], corsa_t **corse_ord[], int *numCorse) {
    static enum ord_type_e ord_type;
    static int depart_ordered = 0;

    switch (comando) {
        case r_stampa:
            stampaLog(corse_ord[ord_type], *numCorse);
            break;
        case r_carica:
            *numCorse = readFile(&corse, corse_ord, 1);
            break;
        case r_ord_data:
            ordinaCorse(corse_ord[data], *numCorse, confrontoData);
            ord_type = data;
            break;
        case r_ord_codice:
            ordinaCorse(corse_ord[codice], *numCorse, confrontoCodice);
            ord_type = codice;
            break;
        case r_ord_partenza:
            ordinaCorse(corse_ord[partenza], *numCorse, confrontoPartenza);
            ord_type = partenza;
            depart_ordered = 1;
            break;
        case r_ord_capolinea:
            ordinaCorse(corse_ord[capolinea], *numCorse, confrontoCapolinea);
            ord_type = capolinea;
            break;
        case r_ricerca:
            // Search always on the array ordered by departure, if it has actually been ordered the search will be dicotomic
            eseguiRicerca(corse_ord[partenza], *numCorse, depart_ordered);
            break;
        default:
            break;
    }
